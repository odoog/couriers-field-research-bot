...
class Stage:
    ...
    def __init__(self,
                 ...
                 prerequisite_actions: Optional[
                     List[PrerequisiteAction] | Callable[..., List[PrerequisiteAction]]] = None,
                 user_input_actions: Optional[
                     List[Action] | Callable[..., List[Action]] | List[Callable[..., Action]] | Callable[
                         ..., List[Callable[..., Action]]]] = None,
                 user_input_filter: Optional[InputFilter | Callable[..., InputFilter]] = None,
                 statistics: Optional[List[Stats]] = None,
                 is_gatehouse: bool = False,
                 # TODO: Автоматическое определение IsGatehouse в зависимости от прикрепленных actions.
                 sending_message_error_actions: Optional[
                     List[PrerequisiteAction] | Callable[..., List[PrerequisiteAction]]] = None):
        self._name = name
    ...
    def is_allowed_input(self,
                         input_string: AnyStr,
                         scope: Scope,
                         user: User) -> (bool, str):  # is_allowed + not_allowed_reason
        if input_filter := self.get_user_input_filter(scope, user):
            if not input_filter.is_allowed_input(input_string):
                return False, input_filter.not_passed_reason_message
            ...
            if keyboard := message.get_keyboard(scope, user):
                if not keyboard.is_non_keyboard_input_allowed:
                    ...
                    if input_string not in keyboard_buttons_strings:
                        return False, "Выберите один из вариантов и нажмите."
        return True, ""
    ...
    def process_input(self,
        ...
        is_allowed_input, not_allowed_reason = self.is_allowed_input(input_string, scope, user)
        ...
        if not is_allowed_input:
            ...
            transition_user_message.set_onetime_text_processor_method(lambda text: not_allowed_reason + "\n\n" + text)
